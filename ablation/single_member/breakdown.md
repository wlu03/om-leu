**Problem:**  
Use one member instead of five: $\bar p_i(j) = p_1(j \mid E_i, z_i)$ instead of $\frac{1}{5}\sum_m p_m$. What does the ensemble buy, and why does the same ablation appear in both the sentence-only and the mixture columns with different sizes?
...
Inside the mixture the same change is worth less. The structural channel does not change, and $\pi$ re-optimises around the single member: a noisier sentence channel receives a smaller weight, and the mixture's own averaging with $q_i$ absorbs part of the member variance. This is the general pattern of `ablation/`: mixture deltas are compressed versions of sentence-only deltas, with the compression factor set by how much of the channel the fitted $\pi$ uses. It also gives the ensemble a fair reading against the plain-MLP baselines, which are averaged over five seeds as well; the design comparison in `plain_nn_sentences` is between two five-member ensembles, so the ensembling itself is not part of the "designed model" credit.
